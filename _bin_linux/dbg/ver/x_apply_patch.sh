...
apply_patch_detail()
{
	...
	ver_now=${ver_raw%__*}
	sed -i "2c \当前版本:   $ver_now"   x_patch_update.log
...
apply_patch()
{
    ...
    if [ $ver_cur = $ver_big ]; then
        ...
        [ ! -f $patch__need_tgz ] && fn_ftp_dn_file HrW/dn  $patch__need_tgz  .
        md5sum_down=`md5sum $patch__need.tgz | awk '{print $1}'`
        if [ $md5sum_need != $md5sum_down ]; then
            echo 校验md5失败 $md5sum_need $md5sum_down
        else
            apply_patch_detail $patch__need_tgz
        fi
    fi
}
